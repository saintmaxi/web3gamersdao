...
const getMerkleProof = async () => {
  ...
  const _proof = await fetch(
    `https://www.significantothers.xyz/.netlify/functions/merkle?addr=${_senderAddress}`
  ).then((res) => res.text());
  try {
...
const checkWhitelistStatus = async () => {
  ...
  const isWhitelisted = await nft
    .verifyPublicAllowList(userAddress, merkleProof)
    .catch((err) => console.log(err));
  if (await checkMintingLive()) {
    if (isWhitelisted) {
      $("#whitelisted").html(
        "you made the allowlist!<br>claim 1 free with the 'mint allowlist' button."
      );
...
const allowlistMint = async () => {
  try {
    ...
    if (!whitelisted) {
      ...
      const merkleProof = await getMerkleProof();
      const cost = 0;
      const gasLimit = await nft.estimateGas.allowListMint(1, merkleProof, {
        value: cost,
      });
      const newGasLimit = parseInt(gasLimit * 1.2).toString();

      await nft
        .allowListMint(1, merkleProof, { gasLimit: newGasLimit })
        .then(async (tx_) => {
...
const publicMint = async () => {
  ...
  try {
    if (minted == MAX_SUPPLY) {
      ...
      const gasLimit = await nft.estimateGas.claim(
        userAddress,
        numberToMint,
      ...
      await nft
        .claim(
          ...
          0,
          { value: cost, gasLimit: newGasLimit }
        )
        .then(async (tx_) => {
          await waitForTransaction(tx_);
        });
...
const updateMintInfo = async () => {
  ...
  if (minted >= MAX_SUPPLY) {
    $("#sold-out").html(
      `Mint sold out. <br><br>available on <a href="${openseaLink}" target="_blank" class="w-inline-block" style="text-decoration:none;color:blue;">Opensea⬈</a> & <a href="${looksrareLink}" target="_blank" class="w-inline-block" style="text-decoration:none;color:green;">Looksrare⬈</a>`
    );
    $("#mint-button").remove();
